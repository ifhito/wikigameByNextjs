...
// Socket.IOサーバーの初期化と設定
function setupSocketIO() {
  if (io) return io;
  
  console.log('Creating new Socket.IO server instance with path /api/socket/io');
  io = new Server({
    cors: {
      origin: '*',
      methods: ['GET', 'POST'],
...
if (process.env.NODE_ENV === 'development') {
  ...
  const socketServer = setupSocketIO();
  try {
...
export async function GET() {
  ...
  return new NextResponse('Socket.IO endpoint is available at /api/socket/io', { status: 200 });
} 